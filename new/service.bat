...
if "%~1"=="load_custom_lists" (
    call :custom_lists_status
    exit /b
)
...
if not defined LISTS set "LISTS=%~dp0lists\"

set "CustomListsStatus=disabled"
...
set "customFlag=%~dp0utils\custom_lists.enabled"
set "cfg=%~dp0utils\custom_lists.cfg"

if not exist "%customFlag%" exit /b
set "CustomListsStatus=enabled"

set "useHostExclude=1"
set "useIpSet=1"
set "useIpSetExclude=1"

if exist "%cfg%" (
    for /f "usebackq tokens=1* delims==" %%A in ("%cfg%") do (
        if /i "%%A"=="HOSTLIST_EXCLUDE" set "useHostExclude=%%B"
        if /i "%%A"=="IPSET" set "useIpSet=%%B"
        if /i "%%A"=="IPSET_EXCLUDE" set "useIpSetExclude=%%B"
    )
)

if exist "%LISTS%custom-hostlist.txt" (
    set "HostListGeneral=%LISTS%custom-hostlist.txt"
    set "HostListGoogle=%LISTS%custom-hostlist.txt"
)

if "%useHostExclude%"=="0" set "HostListExcludeArg="
if "%useIpSet%"=="0" set "IpSetArg="
if "%useIpSetExclude%"=="0" set "IpSetExcludeArg="
exit /b
...
set "tlsFile=%~dp0utils\tls_hello.mode"
set "TlsHelloFile=tls_clienthello_www_google_com.bin"
set "candidate="

if exist "%tlsFile%" (
    for /f "usebackq delims=" %%A in ("%tlsFile%") do (
        set "candidate=%%A"
        goto tls_hello_status_validate
    )
)

:tls_hello_status_validate
if /i "%candidate%"=="tls_clienthello_4pda_to.bin" set "TlsHelloFile=%candidate%"
if /i "%candidate%"=="tls_clienthello_max_ru.bin" set "TlsHelloFile=%candidate%"
if /i "%candidate%"=="tls_clienthello_www_google_com.bin" set "TlsHelloFile=%candidate%"

set "TlsHelloStatus=www.google.com"
if /i "%TlsHelloFile%"=="tls_clienthello_4pda_to.bin" set "TlsHelloStatus=4pda.to"
if /i "%TlsHelloFile%"=="tls_clienthello_max_ru.bin" set "TlsHelloStatus=max.ru"
exit /b
...
set "next=tls_clienthello_www_google_com.bin"
if /i "%TlsHelloFile%"=="tls_clienthello_www_google_com.bin" set "next=tls_clienthello_4pda_to.bin"
if /i "%TlsHelloFile%"=="tls_clienthello_4pda_to.bin" set "next=tls_clienthello_max_ru.bin"
if /i "%TlsHelloFile%"=="tls_clienthello_max_ru.bin" set "next=tls_clienthello_www_google_com.bin"
